...
- 2026-06-01 — Block 9 (Delay Tracking) spec drafted. Four-doc spec in docs/delay_tracking/. Key decisions: approval-lock (approved_by set → immutable, 409 on edit); actual_duration derived not stored; port_call_id XOR leg_ref (both nullable = voyage-level); leg_ref string seam (no Leg entity in V1); no role gate. Two milestones. M1 must start after Block 8 M1 lands.
- 2026-06-01 — Block 8 (Bunker Request) spec drafted. Prompt A/B skipped (no new stack/domain, plain CRUD + status machine). Four-doc spec in docs/bunker_request/. Key decisions: status machine Raised→In Progress→Stemmed→Supplied with Blocked as a side-state from any non-terminal; blocker_note mandatory on Blocked; unblock requires operator to supply target status; no role gate; no delivered-volume fields (trimmed V1). Two milestones.
