...
**Status:** Block 6 (Operational Reporting) **complete**. Block 7 split into **7a (Forms + LLM ingest)** and **7b (Checklists)**; founder confirmed LLM email-to-form parsing stays (phased ingest — paste/submit core now, IMAP seam later). **Block 7a spec drafting in progress** — Prompt A (OSS-fit scan) next.
...
Block 7a (Forms — LLM email-to-form ingest). Draft the five-doc spec set in `docs/forms/` per `[ADR-0012]` (founder approval gate between each doc), then open the M0 coordinator. LLM stack locked by [ADR-0014]. Ingest is phased (paste/submit core now, IMAP seam later) so OPEN_DECISIONS §10 stays deferred. Block 7b (Checklists) follows after 7a closes.
